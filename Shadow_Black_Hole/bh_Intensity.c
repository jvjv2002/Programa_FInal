...
int main(void){
    ...
    const size_t Nx = 200;
    const size_t Ny = 200;
    const double x_interval = 8.0; // -10 -> 10
    const double y_interval = 8.0; // 10 -> 10
    ...
    for (size_t j = 0; j<Ny; j++){
        ...
        for(size_t i = 0; i<Nx; i++){
            ...
            if(xf[1]<r_bh*1.0001){
                // Trajetória capturada
                shadow[Nx*j + i] = 0.0;
            }else{
                // Analisar Posição aproximada de onde veio a partícula
                double r = xf[1];
                double theta = xf[2];
                double phi = xf[3];
                double pr = xf[4];
